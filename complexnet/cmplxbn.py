...
def complexBatchStandardize(input, epsilon=1e-4, polar=False):
    ...
    if ndims == 5:
        ...
        Vrr = torch.mean(torch.mean(input_centered_squared[:, :, :, :, 0], input_axes[0], True),
                         input_axes[1], True) + epsilon
        Vii = torch.mean(torch.mean(input_centered_squared[:, :, :, :, 1], input_axes[0], True),
                         input_axes[1], True) + epsilon
        Vri = torch.mean(torch.mean(input_centered[:, :, :, :, 0] * input_centered[:, :, :, :, 1], input_axes[0], True),
                         input_axes[1], True)
        ...
        Vrr = torch.mean(torch.mean(torch.mean(input_centered_squared[:, :, :, :, :, 0], input_axes[0], True),
                                    input_axes[1], True), input_axes[2], True) + epsilon
        Vii = torch.mean(torch.mean(torch.mean(input_centered_squared[:, :, :, :, :, 1], input_axes[0], True),
                                    input_axes[1], True), input_axes[2], True) + epsilon
        Vri = torch.mean(torch.mean(torch.mean(input_centered[:, :, :, :, :, 0] * input_centered[:, :, :, :, :, 1],
                                               input_axes[0], True), input_axes[1], True), input_axes[2], True)
    ...
    stndrzd_output = torch.stack([brdcst_Wrr, brdcst_Wii], dim=ndims - 1) * input_centered + torch.stack(
        [brdcst_Wri, brdcst_Wri], dim=ndims - 1) * rolled_input
...
class ComplexBatchNormalize(nn.Module):
    ...
    def __init__(self, num_features, epsilon=1e-4, momentum=0.1, center=True, scale=True, track_running_stats=True,
                 polar=False):
    ...
    def normalizeComplexBatch(self, input):
        ...
        if self.scale:
            ...
            rolled_stndrzd_output = torch.stack([stndrzd_output[:, :, :, :, 1], stndrzd_output[:, :, :, :, 0]],
                                                dim=ndims - 1)

            if self.center:
                brdcst_beta = self.beta.unsqueeze(0).unsqueeze(2).unsqueeze(3).unsqueeze(4).expand_as(mu)
                output = torch.stack([brdcst_gamma_rr, brdcst_gamma_ii], dim=ndims - 1) * stndrzd_output + torch.stack(
                    [brdcst_gamma_ri, brdcst_gamma_ri], dim=ndims - 1) * rolled_stndrzd_output + brdcst_beta
            else:
                output = torch.stack([brdcst_gamma_rr, brdcst_gamma_ii], dim=ndims - 1) * stndrzd_output + torch.stack(
                    [brdcst_gamma_ri, brdcst_gamma_ri], dim=ndims - 1) * rolled_stndrzd_output
        else:
            if self.center:
                brdcst_beta = self.beta.unsqueeze(0).unsqueeze(2).unsqueeze(3).unsqueeze(4).expand_as(mu)
